...
NSString* kZipArchiveErrorDomain = @"kZipArchiveErrorDomain";
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error
{
	...
	if (zip == NULL) {
		...
		if (error) {
			*error = [NSError errorWithDomain:kZipArchiveErrorDomain code:-1 userInfo:userInfo];
		}
		return NO;
	}
	...
	if (unzGoToFirstFile(zip) != UNZ_OK) {
		...
		if (error) {
			*error = [NSError errorWithDomain:kZipArchiveErrorDomain code:-2 userInfo:userInfo];
		}
		return NO;
	}

	NSDateComponents *comps = [NSDateComponents new];
    comps.day = 1; comps.month = 1; comps.year = 1980;
	NSCalendar *gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar];
	NSDate *nineteenEighty = [gregorian dateFromComponents:comps];
